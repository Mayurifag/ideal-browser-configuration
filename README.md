...
inb4: Да, это всё вместе уживается и не тормозит у меня в браузере на mid-end конфигурациях лаптопа и пекарни.
...
Опционально потребуется после установок аддонов с помощью [Addons UUID Replacer](https://github.com/Izheil/Quantum-Nox-Firefox-Dark-Full-Theme/releases/) сгенерировать addons.css, чтобы ublock и другие расширения тоже были темными. Для этого понадобится указать путь к профилю в cmd.
